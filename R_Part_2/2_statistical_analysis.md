...
# Define Analysis Window (Only Data that is good enough)
calc_start <- 2000
calc_end   <- 2024

# Baseline Mean out of full Dataset
base_vals <- area_strict_c %>%
  filter(year >= base_start & year <= base_end) %>%
  summarise(
...
# anomalie only for 2000-2024
area_anom <- area_strict_c %>%
  filter(year >= calc_start & year <= calc_end) %>%
  mutate(
...
``` r
cat("Calculated years:", calc_start, "-", calc_end, "\n")
```

    Calculated years: 2000 - 2024 
...
``` r
# Models on anomalies and sensitivity on temperature
m_anom_T   <- lm(area_anom ~ t_anom, data = area_anom)
...
```
    ...
    Residuals:
        Min      1Q  Median      3Q     Max 
    -5.5651 -0.6502  0.5974  1.5070  3.4707 

    Coefficients:
                 Estimate Std. Error t value Pr(>|t|)  
    (Intercept)  0.005269   0.504654   0.010   0.9918  
    t_anom      -1.965083   0.724617  -2.712   0.0131 *
    ---
    Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1

    Residual standard error: 2.382 on 21 degrees of freedom
    Multiple R-squared:  0.2594,    Adjusted R-squared:  0.2241 
    F-statistic: 7.354 on 1 and 21 DF,  p-value: 0.01306
...
```
    ...
    Residuals:
        Min      1Q  Median      3Q     Max 
    -5.6453 -1.3131  0.8792  1.3076  3.1995 

    Coefficients:
                Estimate Std. Error t value Pr(>|t|)   
    (Intercept) -0.04191    0.50453  -0.083  0.93462   
    t_anom      -2.12516    0.73676  -2.884  0.00917 **
    tp_anom     -0.04414    0.04083  -1.081  0.29257   
    ---
    Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1

    Residual standard error: 2.372 on 20 degrees of freedom
    Multiple R-squared:  0.3003,    Adjusted R-squared:  0.2303 
    F-statistic: 4.291 on 2 and 20 DF,  p-value: 0.02814
...
```
    ...
    Residuals:
        Min      1Q  Median      3Q     Max 
    -5.6324 -0.8556  0.5470  1.3129  3.2205 

    Coefficients:
                   Estimate Std. Error t value Pr(>|t|)  
    (Intercept)    -0.01576    0.51436  -0.031   0.9759  
    t_anom         -2.03005    0.76441  -2.656   0.0156 *
    tp_anom        -0.05546    0.04541  -1.222   0.2368  
    t_anom:tp_anom  0.02913    0.04748   0.614   0.5468  
    ---
    Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1

    Residual standard error: 2.41 on 19 degrees of freedom
    Multiple R-squared:  0.3138,    Adjusted R-squared:  0.2055 
    F-statistic: 2.897 on 3 and 19 DF,  p-value: 0.06196
...
```
    ...
    SENSITIVITY: per +1°C (Aug–May), area_anom changes by -1.97 km² (95% CI: -3.47 to -0.46 )
...
p_anom_ts_up <- ggplot(area_anom, aes(x = year, y = area_anom)) +
  ...
  geom_smooth(method = "lm", formula = y ~ x, se = TRUE, color = "#00B8D9") +
  annotate(
    "label",
  ...
  labs(
    title = paste0("Area anomaly of the Fox Glacier (years ", calc_start, "-", calc_end, ")"),
    subtitle = paste0("Baseline = ", base_start, "-", base_end, " : Glacier over/ under reference-niveau (Aug–May)"),
    x = "Year",
...
![](glacier_test_files/figure-commonmark/unnamed-chunk-11-2.png)
